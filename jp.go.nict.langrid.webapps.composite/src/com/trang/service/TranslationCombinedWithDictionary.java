...
public class TranslationCombinedWithDictionary extends
		AbstractTranslationService {

	public TranslationCombinedWithDictionary() {
		setSupportedLanguagePairs(pairs);
	}

	private static Collection<LanguagePair> pairs = new ArrayList<LanguagePair>();
	static {
		LanguagePairUtil.addBidirectionalRoundrobinformedPairs(pairs,
				new Language[] { ar, bg, cs, da, nl, en, et, fi, fr, de, el,
						ht, he, hu, id, it, ja, ko, lv, lt, no, pl, pt_PT, ro,
						ru, sk, sl, es, sv, th, tr, uk, vi, zh, zh_CN, zh_Hans,
						zh_TW, zh_Hant });
	}
	...
	private String invokeTranslation(Language sourceLang, Language targetLang,
			String source) throws Exception {
		String result = "";
		try {
			
			TranslationWithTemporalDictionaryService service = new PbClientFactory()
					.create(TranslationWithTemporalDictionaryService.class,
							new URL(
			...
			req.getTreeBindings()
					.add(new BindingNode(
							"TranslationPL",
							"http://localhost:8080/jp.go.nict.langrid.webapps.composite/pbServices/BPELTwoHopTranslation"));
			result = service.translate(sourceLang.getCode(), targetLang.getCode(), source,
					new Translation[] { new Translation("",
							new String[] { "" }) }, targetLang.getCode());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
